...
설계와 다르게 구현해야 할 이유가 생기면 **코드 수정 전에 이 문서를 먼저 갱신**한다.
...
편집 권한 멤버로 전환한다(선택). 이양 이력은 기능 4(변경 이력)에 기록한다.

### 4-2. DB 변경
전용 테이블 없음. `companies.owner_id` 갱신 + `company_history`(기능 4)에 로그. 기존 owner를
...
Response 403  { error: "권한이 없습니다" }            // 현재 owner/admin 아님
Response 404  { error: "존재하지 않는 사용자입니다" }  // new_owner 없음/비활성
```
- 권한: 현재 owner 또는 admin.
- 트랜잭션: `owner_id` 갱신 → (keep_as_member면 기존 owner를 `company_members` edit로 등록) → `company_history`에 `field='owner_id'` 이력 INSERT.

### 4-4. 화면
업체 상세에 "담당자 이양" 액션(owner/admin 노출). 대상 사용자 선택 → 확인 모달.

### 4-5. 흐름
```
현재 owner/admin → 업체 상세 "담당자 이양" → new_owner 선택
